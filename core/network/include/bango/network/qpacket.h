...
namespace bango { namespace network {
    ...
    public:
        qpacket(unsigned char type) : qpacket()
        {
            m_header[2] = type;
        }
        ...
        qpacket(const std::vector<char>& buffer) : qpacket()
        {
        ...
        template<typename T>
        friend qpacket& operator>> (qpacket& lhs, T& rhs) { rhs = lhs.pop<T>(); return lhs; }
        friend qpacket& operator>> (qpacket& lhs, std::string& rhs) { rhs = lhs.pop_str(); return lhs; }

        friend qpacket& operator<< (qpacket& lhs, std::string& rhs) { lhs.push_str(rhs); return lhs; }
        friend qpacket& operator<< (qpacket& lhs, const char* rhs) { lhs.push_str(rhs); return lhs; }

        void merge(const qpacket& p);
        friend qpacket& operator<< (qpacket& lhs, const qpacket& rhs) { lhs.merge(rhs); return lhs; }
    };

    template<typename T>
    inline void qpacket::push(const T& value)
    {
